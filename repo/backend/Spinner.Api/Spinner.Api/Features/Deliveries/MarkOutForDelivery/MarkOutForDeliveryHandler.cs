...
public sealed class MarkOutForDeliveryHandler
    : IRequestHandler<MarkOutForDeliveryCommand, Result<DeliveryDetailsResponse>>
{
    private readonly AppDbContext _dbContext;

    public MarkOutForDeliveryHandler(AppDbContext dbContext)
    {
        _dbContext = dbContext;
    }

    public async Task<Result<DeliveryDetailsResponse>> Handle(
        MarkOutForDeliveryCommand request,
        CancellationToken cancellationToken)
    {
        ...
        var transition = order.MarkOutForDelivery(DateTimeOffset.UtcNow);
