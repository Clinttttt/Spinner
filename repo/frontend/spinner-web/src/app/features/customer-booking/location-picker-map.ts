...
export class LocationPickerMap implements AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit(): void {
    try {
      ...
      // Satellite imagery with road and place labels drawn over it, and the only
      // view offered. Rural purok addresses have no house numbers, so recognising
      // your own roof is the most reliable way to place the pin; a plain road map
      // shows an empty space where the houses are. Esri's basemaps need no key,
      // which keeps the no-paid-API constraint.
      const tiles = leaflet.tileLayer(
        'https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}',
        { attribution: 'Imagery &copy; Esri', maxNativeZoom: 19, maxZoom: 21 },
      );

      const labels = leaflet.tileLayer(
